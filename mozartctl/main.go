...
func clusterCreate(c *cli.Context) {
  ...
  joinKey := base64.URLEncoding.EncodeToString(randKey)
	...
	fmt.Printf(`docker run --name mozart-agent -d --rm --privileged -v /var/run/docker.sock:/var/run/docker.sock -p 49433:49433 -e "MOZART_SERVER_IP=%s" -e "MOZART_JOIN_KEY=%s" -e "MOZART_CA_HASH=%s" zbblanton/mozart-agent`, server, joinKey, caHash)
	fmt.Printf("\n\n\n")
}
...
func clusterPrint(c *cli.Context) {
	...
	fmt.Printf(`docker run --name mozart-agent -d --rm --privileged -v /var/run/docker.sock:/var/run/docker.sock -p 49433:49433 -e "MOZART_SERVER_IP=%s" -e "MOZART_JOIN_KEY=%s" -e "MOZART_CA_HASH=%s" zbblanton/mozart-agent`, config.ServerIp, config.AgentJoinKey, caHash)
	fmt.Printf("\n\n\n")
}
