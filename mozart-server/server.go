...
func controllerWorkers() {
  ...
  for {
    ...
    for index, worker := range workers.Workers {
      ...
      if(checkWorkerHealth(worker.NodeIp)){
...
func checkWorkerHealth(workerIp string) bool {
  //Will need to add support for the worker key!!!!!
  type Req struct {
    Key string
  }
...
func main() {
  ...
  //Initialize
  //TODO: add logic to see if files exist first
  ...
  //Temp write functions for ^^^ to keep the files from being empty and unable to decode
